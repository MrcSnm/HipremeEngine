// D import file generated from 'source\jni\helper\jnicall.d'
...
version (Android)
{
	enum javaRep = ["byte":"B", "ubyte":"B", "char":"C", "bool":"Z", "int":"I", "uint":"I", "void":"V", "long":"J", "ulong":"J", "float":"F", "double":"D", "short":"S", "ushort":"S", "string":"Ljava/lang/String;", "Object":"Ljava/lang/Object;"];
	...
	enum D_TO_JAVA_TYPE_TABLE = ["ubyte":"jboolean", "byte":"jbyte", "ushort":"jchar", "short":"jshort", "int":"jint", "long":"jlong", "float":"jfloat", "double":"jdouble", "string":"jstring"];
	D javaTypeToD(D, J)(JNIEnv* env, J value)
	{
		...
		else
		{
			...
			else
			{
				...
				{
					jarray obj = value;
					mixin(format!"\x0d\x0a            j%s* javaArr = (*env).Get%sArrayElements(env, cast(j%sArray)obj, null);\x0d\x0a        "(javaType, javaTypeUpper, javaType));
					...
					mixin(format!"(*env).Release%sArrayElements(env, obj, javaArr, 0);"(javaTypeUpper));
					return ret;
				}
			}
		}
	...
	string java_convertParameterToD(string typeName, string varName)()
	{
		...
		{
			return format!"javaTypeToD!string(env, %s)"(varName);
		}
		else
		{
			static if (isTypeArray)
			{
				return format!"javaTypeToD!(%s)(env, %s)"(typeName, varName);
			}
			else
			{
				return varName;
	...
	string java_getParametersDef(string funcParams)()
	{
		...
		static foreach (i, v; argsSplit)
		{
			if (i != 0)
				ret ~= ",";
			ret ~= java_dType_to_javaType!(argsSplit[i].split(" ")[0..$ - 1].join) ~ " " ~ argsSplit[i].split(" ")[$ - 1];
		}
		return ret;
	}
	string java_getParametersCall(string funcParams)()
	{
		...
		static foreach (i, v; paramsTemp)
		{
			...
			mixin(format!"enum string[] typeAndName_%s = v.split(\" \");"(i));
			mixin(format!"enum string type_%s = typeAndName_%s[0..$-1].join;"(i, i));
			mixin(format!"enum string name_%s = typeAndName_%s[$-1];"(i, i));
			paramsCall ~= java_convertParameterToD!(mixin(format!"type_%s"(i)), mixin(format!"name_%s"(i)));
		}
		return paramsCall;
	}
	string javaGenerateMethod(alias javaPackage, string funcSymbol, string m = __MODULE__)()
	{
		...
		return format!"\x0d\x0a        export extern(C) %s %s (JNIEnv* env, jclass clazz, %s)\x0d\x0a        {\x0d\x0a            pragma(inline, true)\x0d\x0a            %s(%s);\x0d\x0a        }\x0d\x0a    "(funcRet, metName, funcParamsConverted, funcSymbol, paramsCall);
	}
	template javaGenerateModuleMethodsForPackage(alias javaPackage, alias module_, bool showGeneration = false)
	{
	...
	template javaGetPackage(string packageName)
	{
		...
		auto javaCall(T, string path, Args...)(JNIEnv* env = _env)
		{
			...
			jmethodID id = (*env).GetStaticMethodID(env, cls, javaGetMethodName(where).toStringz, typeRepresentation.toStringz);
			static if (is(T == Object))
			{
				jobject obj = mixin("(*env).CallStaticObjectMethod(env, cls, id " ~ s ~ ")");
			...
			else
			{
				...
				{
					jstring obj = mixin("(*env).CallStaticObjectMethod(env, cls, id " ~ s ~ ")");
				...
				else
				{
					static if (!isArray!T)
					{
						return cast(T)mixin("(*env).CallStatic" ~ javaTypeUpper ~ "Method(env, cls, id " ~ s ~ ")");
					}
					else
					{
						jarray obj = mixin("(*env).CallStaticObjectMethod(env, cls, id " ~ s ~ ")");
						return javaTypeToD!T(env, obj);
					}
				}
			}
