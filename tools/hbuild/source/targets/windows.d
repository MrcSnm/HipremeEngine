...
ChoiceResult prepareWindows(Choice* c, ref Terminal t, ref RealTimeConsoleInput input, in CompilationOptions cOpts)
{
	...
	// waitOperations([{
		...
		if(timed(t, waitRedub(t, input, DubArguments().command("build").configuration("script").opts(cOpts)) != 0))
			return ChoiceResult.Error;
			// return false;
		// return true;
	// },
	...
	t.wait(spawnProcess([getHipPath("bin", "desktop", "hipreme_engine.exe"), configs["gamePath"].str], ["PATH": environment["PATH"]]));
