module hip.jni.android.sensor;
import hip.jni.android.android_api;
...
struct ASensorEvent {
    int32_t version_; /* sizeof(struct ASensorEvent) */
    int32_t sensor;
    int32_t type;
    int32_t reserved0;
    int64_t timestamp;
...
int ASensorManager_createHardwareBufferDirectChannel(
        ASensorManager* manager, const AHardwareBuffer* buffer, size_t size);
...
int ASensorManager_configureDirectReport(ASensorManager* manager,
        const ASensor* sensor, int channelId, int rate);
}
 ...
 */
int ASensorEventQueue_registerSensor(ASensorEventQueue* queue, const ASensor* sensor,
