// D import file generated from 'source\hip\util\reflection.d'
...
enum hasModule(string modulePath) = is(typeof(()
{
	mixin("import ", modulePath, ";");
}
));
enum hasType(string TypeName) = __traits(compiles, ()
{
	mixin(TypeName, " _;");
}
);
enum getUDAs(alias symbol) = __traits(getAttributes, symbol);
template hasUDA(alias symbol, alias UDA)
{
	enum helper = ()
	{
		...
		{
			if (is(typeof(att) == UDA) || is(att == UDA))
				return true;
		}
		return false;
	}
...
enum isReference(T) = is(T == class) || is(T == interface);
enum hasMethod(T, string method, Params...) = __traits(hasMember, T, method) && is(getParams!(__traits(getMember, T, method)) == Params);
...
ulong enumLength(T)() if (is(T == enum))
{
...
template generateExportFunc(string className, alias funcSymbol)
{
	...
	enum impl = ()
	{
		...
		static if (isRef)
		{
			...
			ret ~= className ~ "." ~ __traits(identifier, funcSymbol) ~ "(" ~ [ParameterIdentifierTuple!funcSymbol].join(",") ~ "));}";
		}
		else
		{
...
template ExportDFunctions(alias mod)
{
	...
	static foreach (mem; __traits(allMembers, mod))
	{
		static if (is(mixin(mem) == class) || is(mixin(mem) == struct))
		{
			static foreach (syms; getSymbolsByUDA!(mixin(mem), ExportD))
			{
				...
				{
					static assert(__traits(compiles, mixin(generateExportName!(mem, syms))), "ExportD '" ~ generateExportName!(mem, syms) ~ "' is not unique, use ExportD(\"SomeName\") for overloading with a suffix");
...
enum ForwardFunc(alias func, string funcName, string member)()
{
	...
	return GetFunctionDeclareStatement!(func, funcName) ~ "{" ~ (!is(ReturnType!func == void) ? "return " : "") ~ member ~ "." ~ funcName ~ "(" ~ [ParameterIdentifierTuple!func].join(",") ~ ");}";
}
...
template GenerateRuntimeAccessors()
{
