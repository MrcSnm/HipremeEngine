...
enum isInteger(T) = isUnsigned!T || isSigned!T;
enum isFloating(T) = is(T == float) || is(T == double);
enum isNumber(T) = isInteger!T || isFloating!T;
enum isStructPointer(T) = is(typeof(*T.init) == struct);
...
class LuaInterpreter : IHipInterpreter
{
    public lua_State* L;
    ...
    public void expose(string name, InterpreterCFunction func)
    {
        expose(name, cast(LuaFunction)func);
    }
