...
static if(IMGUI_USER_CONFIG)
{
    //#include IMGUI_USER_CONFIG
}
static if(!IMGUI_DISABLE_INCLUDE_IMCONFIG_H || IMGUI_INCLUDE_IMCONFIG_H)
{
    import imgui.imconfig;
}
static if(!IMGUI_DISABLE)
{
    //-----------------------------------------------------------------------------
// Header mess
//-----------------------------------------------------------------------------
...
#endif
static if (!IMGUI_USE_STB_SPRINTF  && __clang__ ||__GNUC__)
{
...
extern(C++, ImGui)
{
    ...
    void          Render();                                   // ends the Dear ImGui frame, finalize the draw data. You can get call GetDrawData() to obtain it and run your rendering function (up to v1.60, this used to call io.RenderDrawListsFn(). Nowadays, we allow and prefer calling your render function yourself.)
    ...
    void          ShowMetricsWindow(bool* p_open = NULL);     // create Debug/Metrics window. display Dear ImGui internals: draw commands (with individual draw calls and vertices), window list, basic internal state, etc.
    ...
    void          OpenPopupOnItemClick(const char* str_id = NULL, ImGuiPopupFlags popup_flags = 1);   // helper to open popup when clicked on last item. return true when just opened. (note: actually triggers on the mouse _released_ event to be consistent with popup behaviors)
...
alias IM_ALLOC(_SIZE) =                     ImGui::MemAlloc(_SIZE)
alias IM_FREE(_PTR) =                       ImGui::MemFree(_PTR)
#define IM_PLACEMENT_NEW(_PTR)              new(ImNewWrapper(), _PTR)
#define IM_NEW(_TYPE)                       new(ImNewWrapper(), ImGui::MemAlloc(sizeof(_TYPE))) _TYPE
void IM_DELETE(T)(T* p)   { if (p) { p->~T(); ImGui::MemFree(p); } }
...
struct ImVector(T)
{
    ...
    this(const ref ImVector!T src)                 { Size = Capacity = 0; Data = NULL; operator=(src); }
    ref T opBinary(string op)(const ImVector!T rhs)
    {
        static if(op == "=")
        {
            clear(); resize(src.Size); memcpy(Data, src.Data, cast(size_t)Size * T.sizeof); return *this; 
        }
    }
    ...
    const ref T  front() const                       { IM_ASSERT(Size > 0); return Data[0]; }
    ref T        back()                              { IM_ASSERT(Size > 0); return Data[Size - 1]; }
    const ref T  back() const                        { IM_ASSERT(Size > 0); return Data[Size - 1]; }
    ...
    void         resize(int new_size, const ref T v)    { if (new_size > Capacity) reserve(_grow_capacity(new_size)); if (new_size > Size) for (int n = Size; n < new_size; n++) memcpy(&Data[n], &v, sizeof(v)); Size = new_size; }
    void         shrink(int new_size)                { IM_ASSERT(new_size <= Size); Size = new_size; } // Resize a vector to a smaller size, guaranteed not to cause a reallocation
    ...
    void         push_back(const ref T v)               { if (Size == Capacity) reserve(_grow_capacity(Size + 1)); memcpy(&Data[Size], &v, sizeof(v)); Size++; }
    ...
    T*           insert(const T* it, const ref T v)     { IM_ASSERT(it >= Data && it <= Data + Size); const ptrdiff_t off = it - Data; if (Size == Capacity) reserve(_grow_capacity(Size + 1)); if (off < (int)Size) memmove(Data + off + 1, Data + off, ((size_t)Size - (size_t)off) * sizeof(T)); memcpy(&Data[off], &v, sizeof(v)); Size++; return Data + off; }
    ...
    T*           find(const ref T v)                    { T* data = Data;  const T* data_end = Data + Size; while (data < data_end) if (*data == v) break; else ++data; return data; }
    const T*     find(const ref T v) const              { const T* data = Data;  const T* data_end = Data + Size; while (data < data_end) if (*data == v) break; else ++data; return data; }
    bool         find_erase(const ref T v)              { const T* it = find(v); if (it < Data + Size) { erase(it); return true; } return false; }
    bool         find_erase_unsorted(const ref T v)     { const T* it = find(v); if (it < Data + Size) { erase_unsorted(it); return true; } return false; }
...
extern(C++, ImGui)
{
    ...
    pragma(inline) static void  OpenPopupContextItem(const (char)* str_id = NULL, ImGuiMouseButton mb = 1) { OpenPopupOnItemClick(str_id, mb); } // Bool return value removed. Use IsWindowAppearing() in BeginPopup() instead. Renamed in 1.77, renamed back in 1.79. Sorry!
    ...
    pragma(inline) static bool  DragFloat(const (char)* label, float* v, float v_speed, float v_min, float v_max, const (char)* format, float power)    { return DragScalar(label, ImGuiDataType_Float, v, v_speed, &v_min, &v_max, format, power); }
    pragma(inline) static bool  DragFloat2(const (char)* label, float[2] v, float v_speed, float v_min, float v_max, const (char)* format, float power) { return DragScalarN(label, ImGuiDataType_Float, v, 2, v_speed, &v_min, &v_max, format, power); }
    pragma(inline) static bool  DragFloat3(const (char)* label, float[3] v, float v_speed, float v_min, float v_max, const (char)* format, float power) { return DragScalarN(label, ImGuiDataType_Float, v, 3, v_speed, &v_min, &v_max, format, power); }
    pragma(inline) static bool  DragFloat4(const (char)* label, float[4] v, float v_speed, float v_min, float v_max, const (char)* format, float power) { return DragScalarN(label, ImGuiDataType_Float, v, 4, v_speed, &v_min, &v_max, format, power); }
    ...
    pragma(inline) static bool  SliderFloat(const (char)* label, float* v, float v_min, float v_max, const (char)* format, float power)                 { return SliderScalar(label, ImGuiDataType_Float, v, &v_min, &v_max, format, power); }
    pragma(inline) static bool  SliderFloat2(const (char)* label, float[2] v, float v_min, float v_max, const (char)* format, float power)              { return SliderScalarN(label, ImGuiDataType_Float, v, 2, &v_min, &v_max, format, power); }
    pragma(inline) static bool  SliderFloat3(const (char)* label, float[3] v, float v_min, float v_max, const (char)* format, float power)              { return SliderScalarN(label, ImGuiDataType_Float, v, 3, &v_min, &v_max, format, power); }
    pragma(inline) static bool  SliderFloat4(const (char)* label, float[4] v, float v_min, float v_max, const (char)* format, float power)              { return SliderScalarN(label, ImGuiDataType_Float, v, 4, &v_min, &v_max, format, power); }
    // OBSOLETED in 1.77 (from June 2020)
    pragma(inline) static bool  BeginPopupContextWindow(const (char)* str_id, ImGuiMouseButton mb, bool over_items) { return BeginPopupContextWindow(str_id, mb | (over_items ? 0 : ImGuiPopupFlags_NoOpenOverItems)); }
    // OBSOLETED in 1.72 (from April 2019)
    pragma(inline) static void  TreeAdvanceToLabelPos()               { SetCursorPosX(GetCursorPosX() + GetTreeNodeToLabelSpacing()); }
    // OBSOLETED in 1.71 (from June 2019)
    pragma(inline) static void  SetNextTreeNodeOpen(bool open, ImGuiCond cond = 0) { SetNextItemOpen(open, cond); }
    // OBSOLETED in 1.70 (from May 2019)
    pragma(inline) static float GetContentRegionAvailWidth()          { return GetContentRegionAvail().x; }
    // OBSOLETED in 1.69 (from Mar 2019)
    pragma(inline) static ImDrawList* GetOverlayDrawList()            { return GetForegroundDrawList(); }
    // OBSOLETED in 1.66 (from Sep 2018)
    pragma(inline) static void  SetScrollHere(float center_ratio=0.5f){ SetScrollHereY(center_ratio); }
    // OBSOLETED in 1.63 (between Aug 2018 and Sept 2018)
    pragma(inline) static bool  IsItemDeactivatedAfterChange()        { return IsItemDeactivatedAfterEdit(); }
    ...
    bool      InputFloat2(const (char)* label, float[2] v, int decimal_precision, ImGuiInputTextFlags flags = 0);
    bool      InputFloat3(const (char)* label, float[3] v, int decimal_precision, ImGuiInputTextFlags flags = 0);
    bool      InputFloat4(const (char)* label, float[4] v, int decimal_precision, ImGuiInputTextFlags flags = 0);
    // OBSOLETED in 1.60 (between Dec 2017 and Apr 2018)
    pragma(inline) static bool  IsAnyWindowFocused()                  { return IsWindowFocused(ImGuiFocusedFlags_AnyWindow); }
    pragma(inline) static bool  IsAnyWindowHovered()                  { return IsWindowHovered(ImGuiHoveredFlags_AnyWindow); }
}
alias ImGuiTextEditCallback= ImGuiInputTextCallback;    // OBSOLETED in 1.63 (from Aug 2018): made the names consistent
...
struct ImGuiOnceUponAFrame
{
    ...
    int RefFrame;
    T opCast(T)(){int current_frame = ImGui::GetFrameCount(); if (RefFrame == current_frame) return false; RefFrame = current_frame; return true; }
...
struct ImGuiTextFilter
{
    ...
    struct ImGuiTextRange
    {
        ...
        void  split(char separator, ImVector!ImGuiTextRange* output) const;
    };
    char[256]               InputBuf;
...
{
    ImVector!char      Buf;
    static char[1] EmptyString;

    ImGuiTextBuffer()   { }
    pragma(inline) char opIndex(int index) const
    {
...
struct ImColor
{
    ...
    this(int r, int g, int b, int a = 255)                       { float sc = 1.0f / 255.0f; Value.x = (float)r * sc; Value.y = (float)g * sc; Value.z = (float)b * sc; Value.w = (float)a * sc; }
    this(ImU32 rgba)                                             { float sc = 1.0f / 255.0f; Value.x = (float)((rgba >> IM_COL32_R_SHIFT) & 0xFF) * sc; Value.y = (float)((rgba >> IM_COL32_G_SHIFT) & 0xFF) * sc; Value.z = (float)((rgba >> IM_COL32_B_SHIFT) & 0xFF) * sc; Value.w = (float)((rgba >> IM_COL32_A_SHIFT) & 0xFF) * sc; }
    this(float r, float g, float b, float a = 1.0f)              { Value.x = r; Value.y = g; Value.z = b; Value.w = a; }
    this(const ref ImVec4 col)                                      { Value = col; }
    
    pragma(inline) T opCast(T)()
    {
        static if(is(T == ImU32))
        {
            return ImGui::ColorConvertFloat4ToU32(Value);
        }
        else static if(is(T == ImVec4))
        {
            return Value;
        }
    } 
    ...
    pragma(inline) void    SetHSV(float h, float s, float v, float a = 1.0f){ ImGui::ColorConvertHSVtoRGB(h, s, v, Value.x, Value.y, Value.z); Value.w = a; }
...
static if(!IM_DRAWLIST_TEX_LINES_WIDTH_MAX)
{
